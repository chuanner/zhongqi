...
var pageSize = 20;
//外网
var urlpath = 'http://180.76.181.222:8087/3test';
var imgpath = 'http://180.76.181.222:8087/3test'; 
var imgpath2 = 'http://180.76.181.222:8087/'; 
var urlpic = "http://180.76.181.222:8087";


//外网
//var urlpath = 'http://180.76.181.222:8084/3testbeta';
//var imgpath = 'http://180.76.181.222:8084/3testbeta'; 
//var imgpath2 = 'http://180.76.181.222:8084/'; 
//var urlpic = "http://180.76.181.222:8084";

//内网
//var urlpath = 'http://172.16.0.223:8080/jeesite';
//var imgpath = 'http://172.16.0.223:8080';  
//var imgpath2 = 'http://172.16.0.223:8080'; 
//var urlpic = "http://172.16.0.223:8080";
...
{
	mui.ajax(urlpath + '/a/dataCompare/bdHgz/getAPPTcpageList', {
		type: 'post', //HTTP请求类型
		success: function(data) {
		 ddl = data;
		  localStorage.setItem("ddlCach",JSON.stringify(data));
		  mui.toast('数据同步成功');
		},
...
function findProjnameList()
{
	...
	for(var i=0;i<data.length;i++)
	{
		...
		{
			if(obj.projname == names[j])isExist = true;
		}
		if(isExist == false)names.push(obj.projname);		
	}
	return names;
}

function findXhByProjectname(projectname)
{
	var data = getDDLData();	
	var names = [];
	for(var i=0;i<data.length;i++)
	{
		var obj = data[i];
		if(obj.projname == projectname)
		{
			var isExist = false;
			for(j=0;j<names.length;j++)
			{
				if(obj.anmodel == names[j])isExist = true;
			}
			if(isExist == false)names.push(obj.anmodel);	
		}
	}
	return names;	
	
}
function findBbhByProjectAndXh(projectname,anmodel)
{
    var data = getDDLData();
	var names = [];
	for(var i=0;i<data.length;i++)
	{
		var obj = data[i];
		if(obj.projname == projectname && obj.anmodel == anmodel)
		{
			var isExist = false;
			for(j=0;j<names.length;j++)
			{
				if(obj.editioncode == names[j])isExist = true;
			}
			if(isExist == false)names.push(obj.editioncode);	
		}
	}
	return names;
}

function findPcodeByPxb(projectname,anmodel,editioncode)
{
	 var data = getDDLData();
	var names = [];
	for(var i=0;i<data.length;i++)
	{
		var obj = data[i];
		if(obj.projname == projectname && obj.anmodel == anmodel && obj.editioncode == editioncode)
		{
			var isExist = false;
			for(j=0;j<names.length;j++)
			{
				if(obj.pagecode == names[j])isExist = true;
			}
			if(isExist == false)names.push(obj.pagecode);	
		}
	}
	return names;
}
...
function getDDLData()
{
	if(ddl && ddl.length>0)return ddl;
	 var data;
	 var strData = localStorage.getItem("ddlCach");
	 if(strData==null)
	 {
		data = [];
	 }else{
